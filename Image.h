...
public:
	...
	/// 이미지를 잘라 애니메이션 이미지를 생성하여 리턴
	AnimationImage* createAnimate(float centerX, float centerY, int frameCol, int frameRow);
	...
	void render(HDC hdc, int destX, int destY, BYTE alpha = 255);
	/// 위치 렌더링 : 화면에 그릴 위치 x, y, 원본이미지의 상단 좌측 좌표 x, y, 잘라낼 폭, 잘라낼 높이
	void render(HDC hdc, int destX, int destY, int sourX, int sourY, int width, int height, BYTE alpha = 255);

	/// 크기 렌더링 : 화면에 그릴 위치 x, y, 원본이미지의 상단 좌측 좌표 x, y, 잘라낼 폭, 잘라낼 높이
	void render(HDC hdc, int destX, int destY, int destWidth, int destHeight, int sourX, int sourY, int sourWidth, int sourHeight, BYTE alpha = 255);

	void sizeRender(HDC hdc, int destX, int destY, int width, int height, BYTE alpha = 255);
